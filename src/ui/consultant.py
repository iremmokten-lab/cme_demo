...
def _save_upload(project_id: int, dataset_type: str, file_name: str, file_bytes: bytes, user_id: int | None):
    """
    DEDUP: Aynı project + type + sha256 varsa tekrar kayıt açma.
    """
...
def consultant_app(user):
    ...
    with st.sidebar:
        ...
        psel = st.selectbox("Proje", labels, index=idx)
    ...
    # Raporlar / İndir
    with tabs[3]:
        h2("Raporlar / İndir", "PDF + ZIP/XLSX/JSON indir")
        ...
        if isinstance(cbam_table, list) and cbam_table:
            try:
                ...
                if "cbam_cost_eur" in df.columns:
                    ...
                    df["risk"] = 0.0
                df["eu_tco2"] = df["embedded_t"] if "embedded_t" in df.columns else 0.0
        ...
        if not reports:
            ...
            for r in reports:
                ...
                with col1:
                    st.write(f"📄 Report id={r.id} | {r.created_at}")
                ...
                with col2:
                    ...
                    if p.exists():
                        ...
                        st.warning("PDF dosyası bulunamadı (reboot sonrası silinmiş olabilir).")

    # Senaryolar
    with tabs[4]:
        h2("Senaryolar", "Before/After CBAM € ve ETS TL kıyaslama")
