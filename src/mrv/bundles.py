...
def _sorted_list_of_dicts(rows: List[Dict[str, Any]], keys: List[str]) -> List[Dict[str, Any]]:
    ...
    # Python 3.13: generator expression must be parenthesized when using key=...
...
class InputBundle:
    ...
    def to_canonical_dict(self) -> Dict[str, Any]:
        factors = _sorted_list_of_dicts(
            [f.to_dict() for f in (self.factor_set_ref or [])],
            keys=["factor_type", "region", "version", "id"],
        )
        products = _sorted_list_of_dicts(
            self.product_mapping or [],
            keys=["cn_code", "product_name", "product_code"],
        )
        return {
...
class ResultBundle:
    ...
    def to_canonical_dict(self) -> Dict[str, Any]:
        ...
        cc = _sorted_list_of_dicts(
            [c.to_dict() for c in (self.compliance_checks or [])],
            keys=["reg_reference", "severity", "rule_id"],
        )
        return {
            "engine_version": self.engine_version,
