...
from src.services.projects import get_project_for_user, require_company_id
...
def _is_verifier(user: User) -> bool:
    return str(getattr(user, "role", "") or "").lower().startswith("verifier")


def _can_write(user: User) -> bool:
    # Faz 2: verifier role write yetkili (case/finding/sampling)
    return _is_consultant(user) or _is_verifier(user)
...
def list_cases_for_user(user: User, *, project_id: int | None = None, limit: int = 200) -> List[VerificationCase]:
    """RLS:
    - Consultant / Verifier / Client: kendi company’si altındaki project’lerdeki caseleri görebilir.
    - Client: read-only.
    """
    ...
    with db() as s:
        ...
        return s.execute(q.order_by(VerificationCase.created_at.desc()).limit(int(limit))).scalars().all()
...
def create_case(
    ...
    *,
    ...
    if not _can_write(user):
        raise PermissionError("Sadece danışman/verifier kullanıcılar verification case oluşturabilir.")
...
def update_case_sampling(
    ...
    *,
    ...
    if not _can_write(user):
        raise PermissionError("Sadece danışman/verifier kullanıcılar sampling güncelleyebilir.")
    ...
    with db() as s:
        ...
        c_db.sampling_notes = _safe_str(sampling_notes, "")
        c_db.sampling_size = _safe_int(sampling_size, None)
        s.add(c_db)
        s.commit()
        s.refresh(c_db)
        c = c_db
    ...
    append_audit(
        "case_sampling_updated",
        {
            "case_id": int(case_id),
            "sampling_size": _safe_int(sampling_size, None),
...
def add_finding(
    ...
    *,
    ...
    if not _can_write(user):
        raise PermissionError("Sadece danışman/verifier kullanıcılar bulgu ekleyebilir.")
    ...
    stt = _safe_str(status).lower().strip() or "open"
    if stt not in ("open", "in_progress", "closed"):
        stt = "open"
...
def close_finding(user: User, *, finding_id: int) -> VerificationFinding:
    if not _can_write(user):
        raise PermissionError("Sadece danışman/verifier kullanıcılar bulgu kapatabilir.")
...
def update_case_status(user: User, *, case_id: int, status: str) -> VerificationCase:
    if not _can_write(user):
        raise PermissionError("Sadece danışman/verifier kullanıcılar case durumunu değiştirebilir.")

    c = read_case_for_user(user, int(case_id))
    if not c:
        raise PermissionError("Case bulunamadı veya erişim yok.")

    stt = _safe_str(status).lower().strip() or "planning"
    if stt not in ("planning", "fieldwork", "findings", "closed"):
        stt = "planning"
    ...
    append_audit(
        "case_closed" if stt == "closed" else "case_status_changed",
        {
            "case_id": int(case_id),
            "status": stt,
            "updated_at": _utcnow_iso(),
...
def case_to_json(case: VerificationCase) -> Dict[str, Any]:
    ...
    return {
        ...
        "sampling_size": getattr(case, "sampling_size", None),
