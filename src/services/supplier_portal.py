...
from src.db.global_ready_models_step1 import Producer, ProducerAttestation


def create_producer(company_id: int, name: str, country: str = "TR", vat_or_tax_id: str = "", contact_email: str = "") -> Producer:
    with db() as s:
        p = Producer(company_id=int(company_id), name=str(name).strip(), country=str(country).strip(), vat_or_tax_id=str(vat_or_tax_id).strip(), contact_email=str(contact_email).strip())
        s.add(p)
        s.commit()
        s.refresh(p)
        return p
...
def list_producers(company_id: int) -> list[Producer]:
    with db() as s:
        return (
            s.execute(select(Producer).where(Producer.company_id == int(company_id)).order_by(Producer.id.desc()))
            .scalars()
            .all()
        )
