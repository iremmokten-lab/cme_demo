...
def main():
    ...
    with colB:
        st.write("")
    with colC:
        if st.button("Registry Cache Reset (Uygulama Yenile)", type="secondary"):
            # Streamlit uygulama içinde cache reset: en güvenlisi rerun
            st.success("Cache reset için uygulama yenilenecek.")
            st.rerun()
    ...
    with st.form("add_rule"):
        ...
        with c3:
            good_key = st.selectbox("CBAM Good", options=[k for k, _ in goods], format_func=lambda k: f"{goods_map.get(k, k)} ({k})")
        with c4:
    ...
    with st.form("edit_rule"):
        ...
        with e2:
            match_type_edit = st.selectbox("Match Type", options=["prefix", "exact"], index=0 if (row.match_type or "prefix") == "prefix" else 1)
        with e3:
            good_key_edit = st.selectbox(
                "CBAM Good",
                options=[k for k, _ in goods],
                index=[k for k, _ in goods].index((row.cbam_good_key or "other").strip().lower()) if (row.cbam_good_key or "other").strip().lower() in [k for k, _ in goods] else [k for k, _ in goods].index("other"),
